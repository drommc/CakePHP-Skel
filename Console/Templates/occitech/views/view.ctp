...
<div class="<?php echo $pluralVar; ?> view">
	<div class="page-header">
		<h1><?php echo "<?= h(\${$singularVar}['{$modelClass}']['{$displayField}']); ?>"; ?></h1>
	</div>
	<dl>
<?php
...
<div class="actions">
	...
	<ul>
<?php
	echo "\t\t<?= \$this->Actions->setActionsOptions(\${$singularVar}['{$modelClass}']['{$primaryKey}']); ?>\n";
	echo "\t\t<li><?= \$this->Actions->edit(); ?></li>\n";
	echo "\t\t<li><?= \$this->Actions->deletePost(null, __('Are you sure you want to delete the {$singularHumanName} #%s?', \${$singularVar}['{$modelClass}']['{$primaryKey}'])); ?></li>\n";
	echo "\t\t<li><?= \$this->Actions->index(); ?></li>\n";
	echo "\t\t<li><?= \$this->Actions->add(); ?></li>\n";
...
if (!empty($associations['hasOne'])) :
	...
	<div class="related">
		<h3><?php echo "<?= __('Related " . Inflector::humanize($details['controller']) . "'); ?>"; ?></h3>
	...
	<?php echo "<?php endif; ?>\n"; ?>
		<div class="actions">
			<ul>
				<li><?php echo "<?= \$this->Html->link(__('Edit " . Inflector::humanize(Inflector::underscore($alias)) . "'), array('controller' => '{$details['controller']}', 'action' => 'edit', \${$singularVar}['{$alias}']['{$details['primaryKey']}'])); ?></li>\n"; ?>
			</ul>
		</div>
	</div>
	<?php
...
foreach ($relations as $alias => $details):
	...
	?>
<div class="related">
	<h3><?php echo "<?= __('Related " . $otherPluralHumanName . "'); ?>"; ?></h3>
	<?php echo "<?php if (!empty(\${$singularVar}['{$alias}'])): ?>\n"; ?>
...
<?php echo "<?php endif; ?>\n\n"; ?>
	<div class="actions">
		<ul>
			<li><?php echo "<?= \$this->Actions->add(__('New " . Inflector::humanize(Inflector::underscore($alias)) . "'), '{$details['controller']}'); ?>"; ?></li>
		</ul>
	</div>
</div>
<?php endforeach; ?>
